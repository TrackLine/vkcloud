...
- 🔍 Автоматический поиск Floating IP из указанной подсети
...
- `VKCLOUD_TARGET_NET` - целевая подсеть для поиска IP (по умолчанию: `95.163.248.0/22`)
...
1. **Старт поиска** - когда скрипт начинает работу
2. **Успешная привязка IP** - когда найден и привязан подходящий IP
3. **Ошибка** - когда все воркеры завершились без результата
4. **Остановка** - когда скрипт остановлен пользователем
